...
Before seeing this example, I would have said that `centres_x`, `centres_y` and `marker` would not be 'garbage collected' and the variables would not be preserved. However, the fact the program works means that those variables are preserved by virtue of the fact they are used in the `update` function (and the fact that `update` is returned by `initialise`).
